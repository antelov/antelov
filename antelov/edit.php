...
<style>
    ...
    #updateProfileBtn, #pwdBtn {
        background-color: rgb(240,136,67);
        font-size: 18px;
        width: 150px;
    }
    #deactivateBtn {
...
<div class='profile-page-wrapper'>
    ...
    <div class='profile-page-content'>  
        ...
        <?php 
            if(isset($_SESSION['user'])) {
                $userdata = json_decode($_SESSION['user'], true);
                // var_dump($userdata['uid'], $userdata['user_status'], $userdata['account_status'], $userdata['account_type']);
                
                echo $user->profile_section_1($userdata['uid'], $userdata['user_status'], $userdata['account_status'], $userdata['account_type']); 
                echo $user->profile_section_2($userdata['uid'], $userdata['user_status'], $userdata['account_status'], $userdata['account_type']); 
            }
        ?>
    </div>
</div>
...
<script>
        ...
        if(pwdError.innerHTML == '') {
            if(old_pwd.value && new_pwd.value && repeat_pwd.value) {
                $.ajax({
                    ...
                    async : false, // enable or disable async (optional, but suggested as false if you need to populate data afterwards)
                    success: function(response, textStatus, jqXHR) {
                        window.location.href = './user-profile?id='+user_id;
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.log(jqXHR);
                        console.log(textStatus);
    ...
    if(typeof(ageInp) != 'undefined' && ageInp != null) {
        ageInp.addEventListener('change', function() {
            if(!ageInp.value || (ageInp.value >= 5 && ageInp.value <= 200)) {
    ...
    function removeImg(event) {
        ...
        $('#no-photo-avi').attr('src', './img/avi.png');
        pfpRemoveBtn.style.display = 'none';
        pfpBtn.style.display = 'flex';
